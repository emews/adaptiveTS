...
def create_grid_flow_CRNGP(p,nparam, nrep, model, ref, err_sig,lhs_seed = None):
    r'''
...
def TSBatchBO_CRNGP(init_npar,nrep,p,sim_budget,grid_npar,nTS_samp,
                    ...
                    sim_func = None, exp_seed = None, model = None):
    r'''
    ...
    if adaptive:
        tt = 2
        with tqdm(total=sim_budget,initial=no_of_sims,desc="Processing Data", bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}') as pbar:
            while no_of_sims < sim_budget:
                ...
                model = model_reinit.copy()
                model.fit(Xs,Y)
                
                no_of_sims += len(ynew)
                pbar.set_description(f"Iter {tt}, batch size: {len(ynew)}")
                pbar.update(xnew.shape[0])
                tt += 1
    return {"X_list":X_list, "y_list":y_list, "ynative_list":ynative_list, "simout_list":simout_list}


def next_eval_CRN(model,p,
                  Xgrid = None,evaluated_ids = None,nTS_samp = None,
                  adaptive = None,grid_npar = None,nrep = None,ref = None,err_sig = None,prop_sig = None ,use_flow = None):
    r'''
    Get next evaluations under the emulator
...
def adaptive_CRN_TS(model,p,grid_npar,nrep,ref,err_sig,prop_sig,nTS_samp,use_flow=True):
    if use_flow:
        Xsgrid = create_grid_flow_CRNGP(p=p,nparam=grid_npar,nrep=nrep,
                                      model=model,ref=ref,err_sig=err_sig)
    # setup kwargs?    
    tTS = model.sample(Xsgrid,size=nTS_samp)
