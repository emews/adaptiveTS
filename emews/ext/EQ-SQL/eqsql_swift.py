...
def _create_eqsql(retry_threshold: int = 0, log_level=logging.WARN):
    ...
    return local_queue.init_task_queue(
        host, user, port, db_name, password, retry_threshold, log_level
    )
...
def query_task(
    ...
    try:
        ...
        # result is a msg map
        msg_map = eq_sql.query_task(
            eq_work_type, worker_pool=worker_pool, timeout=query_timeout
        )
...
def query_tasks_n(
    ...
    while _go:
        ...
        try:
            ...
            running_task_ids, tasks = eq_sql.query_more_tasks(
                work_type,
                running_task_ids,
            ...
            if eq_sql is None:
                print(
                    f"eq_swift.query_task_n error {traceback.format_exc()}", flush=True
                )
            else:
                eq_sql.logger.error(
                    f"eq_swift.query_task_n error {traceback.format_exc()}"
                )
            running_task_ids = []
