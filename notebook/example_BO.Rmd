...
```{r}
ytrue_err <- run_sim_err(c(xtrue, strue))
ytrue_err
```
...
for (i in 1:(init_npar*nrep)){
  y[i] <- run_sim_err(Xs_01[i, ])
}
...
TSBatchBO(init_npar, nrep, p, sim_budget = 200, 
          grid_npar = 100, nTS_samp = 30, nTS_iter = 5,
          ytrue = ytrue_full, GP_type = "CRNGP", ref = ref,
