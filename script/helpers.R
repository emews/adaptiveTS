...
threshold_counts <- function(out_list, thresholds=c(15, 20, 25, 30)) {
  outs <- names(out_list)[1:4]
  all_results <- list()
  k = 1
...
checkpointing_speed <- function(out_list, n_trajectories=c(10, 25, 50), thresholds=c(15, 20, 25, 30)) {
  outs <- names(out_list)[1:4]
  all_results <- list()
  k = 1
  for (i in seq_along(outs)) {
...
threshold_budget_analysis <- function(out_list, thresholds=c(15, 20, 25, 30), show_plots=FALSE) {
  outs <- names(out_list)[1:4]
  all_results <- list()
  budget_results <- list()
  k = 1
...
analyze_experiment <- function(exp_id, exp_path, config, thresholds=c(15, 20, 25, 30), n_trajectories=c(10, 25, 50)) {
  out_list <- load_outputs(exp_id, exp_path, config)
  # threshold analysis
  df_threshold <- threshold_counts(out_list, thresholds=thresholds)
  df_checkpoint <- checkpointing_speed(out_list, n_trajectories=n_trajectories, thresholds=thresholds)
  budg <- threshold_budget_analysis(out_list, thresholds=thresholds)
...
search_space <- function(out_list){
  all_results <- list()
  outs <- names(out_list)[1:4]
  for (i in seq_along(outs)) {
    out_name <- outs[i]
